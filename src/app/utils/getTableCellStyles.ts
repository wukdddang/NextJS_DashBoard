...
export const getTableCellStyles = <T>(cell: Cell<T, unknown>) => {
  ...
  let cellStyle = { fontSize: '14px', color: textColor, fontWeight: 'normal' };

  if (
    cell.column.id === 'mag' &&
    parseFloat(cell.getValue() as string) >= 5.0
  ) {
    cellStyle = { ...cellStyle, fontWeight: 'bold', color: 'red' };
  }  else if (
